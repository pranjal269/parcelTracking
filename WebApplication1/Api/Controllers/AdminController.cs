...
namespace WebApplication1.Api.Controllers
{
    ...
    public class AdminController : ControllerBase
    {
        ...
        public AdminController(ApplicationDbContext context, IPasswordHashingService passwordHashingService)
        {
            _context = context;
        ...
        // POST: api/Admin/init-passwords - Initialize default passwords (temporary endpoint)
        [HttpPost("init-passwords")]
        public async Task<IActionResult> InitializePasswords()
        {
            var adminUser = await _context.Users.FirstOrDefaultAsync(u => u.Email == "admin@parcel.com");
            if (adminUser != null)
            {
                adminUser.Password = "password123";
            }

            var handlerUser = await _context.Users.FirstOrDefaultAsync(u => u.Email == "handler@parcel.com");
            if (handlerUser != null)
            {
                handlerUser.Password = "password123";
            }

            await _context.SaveChangesAsync();
            return Ok(new { message = "Default passwords initialized successfully" });
        }
